...
def animatePeriodic(i):
    ...
    fields[0].set_data(rabbits[:, 0],rabbits[:, 1])
    fields[1].set_data(predator[0:2])
    return fields,rect
...
if __name__ == '__main__':
    ...
    for index in range(2):
        lobj = ax.scatter([],[],lw=2,color=plotcols[index])[0]
        fields.append(lobj)
        
    fields[0].set_markersize(ms)
    fields[1].set_markersize(ms)
