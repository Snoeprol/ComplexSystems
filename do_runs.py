...
def make_fig(U, V, color = 'viridis', dpi = 500, output_dir = 'data'):
    ...
    # plt.savefig(f'{output_dir}/N_{N}Nit{Nit}a{alpha}b{beta}c{gamma}DU{D_U}DV{D_V}dt{dt}cmap{color}.png', dpi = dpi)
    plt.clf()
...
def make_fig_clean(U, V, color = 'viridis', dpi = 500, output_dir = 'data'):
    ...
    plt.savefig(f'{output_dir}/N_{N}Nit{Nit}a{alpha}b{beta}c{gamma}DU{D_U}DV{D_V}dt{dt}cmap{color}.png', dpi = dpi)
    size = os.path.getsize(f'{output_dir}/N_{N}Nit{Nit}a{alpha}b{beta}c{gamma}DU{D_U}DV{D_V}dt{dt}cmap{color}.png')
    return size
...
with open(f'{beta}-{gamma}-size.p', 'rb') as handle:
